...
// Replace PLACES_PRESET
appJs = appJs.replace(/const PLACES_PRESET = \[[\s\S]*?\];/m, presetStr);

// Replace fallback block in carregarLocaisDoBackend
const tryCatchRegex = /} catch \(erro\) {[\s\S]*?}/;
const newCatch = `} catch (erro) {
...
appJs = appJs.replace(tryCatchRegex, newCatch);

fs.writeFileSync('app.js', appJs);
console.log('app.js patched to include the 58 places natively!');
